...
import { ChevronLeft, ChevronRight, CircleAlert, CircleCheck, Clipboard, Download, ExternalLink, FileCode2, Filter, Folder, Info, LoaderCircle, Menu, Search, Settings, Trash2, X } from "lucide-react";
import { api, clearToken, downloadExport, type ModelsHealth } from "../services/api";
...
type Run = { id: string; timestamp: string; operation: "Ask" | "Compare" | "Wiki"; question: string; providerModel: string; status: string; latency: number; evidence: Evidence[]; wikiContext: WikiContext[]; answer?: string; comparison?: CompareResult[]; execution?: ExecutionDetails; compareSummary?: CompareRunSummary; evaluationId?: string };

const providerOrder = ["gemini", "openrouter", "openai", "groq", "ollama"];
const providerNames: Record<string, string> = { gemini: "Gemini", openrouter: "OpenRouter", groq: "Groq", ollama: "Local Ollama", openai: "OpenAI" };
...
export default function ProjectWorkspace() {
  ...
  useEffect(() => {
    if (!project || ["indexed", "failed"].includes(project.status)) return;
    let cancelled = false;
    const poll = async () => {
      try {
  ...
  useEffect(() => {
    ...
    setSelectedCompareProviders(current => current.length ? current : available.slice(0, 2));
  }, [modelsHealth]);
  ...
  function modelAvailable(name: string) {
    ...
    if (selected && ["ollama", "groq"].includes(baseProvider(name))) return Boolean(health?.available_models?.includes(selected));
    return Boolean(health?.available ?? health?.status === "Ready");
  }
  ...
  const compareChoices = useMemo(() => Array.from(new Set([
    "gemini", "openrouter", "openai",
    ...((modelsHealth?.groq?.available_models || []).length ? modelsHealth!.groq.available_models.map(name => `groq::${name}`) : ["groq"]),
    ...((modelsHealth?.ollama.available_models || []).length ? modelsHealth!.ollama.available_models.map(name => `ollama::${name}`) : ["ollama"]),
  ])), [modelsHealth]);
  const selectedModelAvailable = modelAvailable(provider);
  const anyModelAvailable = compareChoices.some(name => modelAvailable(name));
  ...
  function restoreRun(run: Run) {
    setQuestion(run.question); setEvidence(run.evidence); setWikiContext(run.wikiContext); setExecution(run.execution || null); setAnswer(run.answer || ""); setAnswerId(run.evaluationId || ""); setComparison(run.comparison || []); setCompareSummary(run.compareSummary || null);
  ...
  return <main className={`product-shell ${sourceOpen ? "source-visible" : ""} ${sidebarOpen ? "sidebar-visible" : ""}`} style={{ "--source-width": `${sourceWidth}px`, "--sidebar-width": `${sidebarWidth}px` } as CSSProperties}>
    <header className="app-header">
      ...
      <Link className="brand brand-link" to="/" aria-label="Security CodeWiki home"><strong>Security CodeWiki</strong><span>{project?.name || "Project"} · {project?.status === "indexed" ? "Indexed" : project?.status || "Loading"} · {project?.files_indexed ?? 0} files · {project?.chunks_indexed ?? 0} chunks</span></Link>
    ...
    <section className="workflow-area">
      ...
      {["ask", "security-wikis"].includes(tab) && !selectedModelAvailable && <div className="grounding-warning" role="status"><CircleAlert/><span><strong>{anyModelAvailable ? "Selected model unavailable." : "No language model is currently available."}</strong> Configure its cloud API key, or start Ollama and install the exact local model. SecurityCodeWiki will not substitute another model.</span></div>}
      {tab === "compare" && !anyModelAvailable && <div className="grounding-warning" role="status"><CircleAlert/><span><strong>No language model is currently available.</strong> Deterministic repository features remain usable. Configure a provider before running Compare.</span></div>}
      <div className="workflow-scroll">
        ...
        {tab === "security-wikis" && <WikiView pages={wikiPages} viewed={viewedWiki} setViewed={setViewedWiki} focus={analysisFocus} model={displayModel(provider, effectiveModel)} generate={generateWiki} remove={deleteWiki} openSource={openSource} disabled={Boolean(busy) || !selectedModelAvailable} unavailable={!selectedModelAvailable} />}
        {tab === "ask" && <AskView question={question} setQuestion={setQuestion} run={runAsk} model={displayModel(provider, effectiveModel)} focus={analysisFocus} answer={answer} evidence={evidence} wikiContext={wikiContext} execution={execution} answerId={answerId} projectId={projectId} openSource={openSource} activeEvidence={activeEvidence} disabled={Boolean(busy) || !selectedModelAvailable} unavailable={!selectedModelAvailable} noModels={!anyModelAvailable} />}
        {tab === "compare" && <CompareView question={question} setQuestion={setQuestion} run={runCompare} providers={compareChoices} available={modelAvailable} reason={(name: string) => modelHealth(name)?.reason} modelName={configuredModel} selected={selectedCompareProviders} setSelected={setSelectedCompareProviders} focus={analysisFocus} results={comparison} evidence={evidence} wikiContext={wikiContext} summary={compareSummary} projectId={projectId} openSource={openSource} activeEvidence={activeEvidence} disabled={Boolean(busy) || !selectedCompareProviders.length} noModels={!anyModelAvailable} />}
...
function Evaluation({ projectId, evaluationId }: { projectId: string; evaluationId: string }) {
  const [values,setValues]=useState<Record<string,string>>({}); const [state,setState]=useState<"idle"|"loading"|"saving"|"saved"|"error">("loading"); const [feedback,setFeedback]=useState("");
  const fromRow=(row:Record<string,any>)=>setValues({correctness:row.correctness_score?.toString()||"",evidence:row.evidence_discipline_score?.toString()||"",completeness:row.completeness?.toString()||"",explanation:row.explanation_quality?.toString()||"",references:row.source_reference_accuracy_score?.toString()||"",usefulness:row.usefulness?.toString()||"",hallucination:row.hallucination_flag==null?"":Boolean(row.hallucination_flag).toString(),verdict:row.verdict||"",notes:row.evaluator_comment||row.human_comment||""});
  useEffect(()=>{let active=true;setState("loading");api.evaluation(projectId,evaluationId).then(row=>{if(active){fromRow(row);setState("idle");}}).catch(cause=>{if(active){setState("error");setFeedback(cause instanceof Error?cause.message:"Could not load evaluation.");}});return()=>{active=false};},[projectId,evaluationId]);
  async function save(){setState("saving");setFeedback("");try{const row=await api.scoreEvaluation(projectId,evaluationId,{correctness:numberOrNull(values.correctness),evidence_discipline:numberOrNull(values.evidence),completeness:numberOrNull(values.completeness),explanation_quality:numberOrNull(values.explanation),source_reference_accuracy:numberOrNull(values.references),usefulness:numberOrNull(values.usefulness),hallucination:values.hallucination===""?null:values.hallucination==="true",verdict:values.verdict||null,notes:values.notes||null});fromRow(row);setState("saved");setFeedback("Evaluation saved.");}catch(cause){setState("error");setFeedback(cause instanceof Error?cause.message:"Evaluation could not be saved.");}}
  return <div className="evaluation-form">{[["correctness","Correctness","0–3: correctness for question scope"],["evidence","Evidence discipline","0–3: grounding and stated uncertainty"],["completeness","Completeness","0–3: coverage of required answer"],["explanation","Explanation quality","0–3: clarity and precision"],["references","Source-reference accuracy","0–2: correct and inspectable references"],["usefulness","Usefulness","0–3: practical auditing value"]].map(([key,label,help])=><label key={key}>{label}<small>{help}</small><select value={values[key]||""} onChange={e=>setValues(current=>({...current,[key]:e.target.value}))}><option value="">Not scored</option>{Array.from({length:key==="references"?3:4},(_,i)=><option key={i} value={i}>{i}</option>)}</select></label>)}<label>Hallucination<select value={values.hallucination||""} onChange={e=>setValues(c=>({...c,hallucination:e.target.value}))}><option value="">Not assessed</option><option value="false">No</option><option value="true">Yes</option></select></label><label>Verdict<select value={values.verdict||""} onChange={e=>setValues(c=>({...c,verdict:e.target.value}))}><option value="">Select</option>{["Verified","Incomplete","Incorrect","Needs Review"].map(v=><option key={v}>{v}</option>)}</select></label><label className="notes">Evaluator notes<textarea value={values.notes||""} onChange={e=>setValues(c=>({...c,notes:e.target.value}))}/></label><button disabled={state==="saving"||state==="loading"} onClick={save}>{state==="saving"?"Saving…":"Save evaluation"}</button>{feedback&&<p className={`evaluation-feedback ${state}`}>{feedback}</p>}</div>;
}
...
function IndexStatus({ project }: { project: Project | null }) {
  const status = project?.status || "loading";
  const complete = status === "indexed";
  const failed = status === "failed";
  const percent = project?.progress_percent;
  const title = complete ? "Indexed successfully" : failed ? "Indexing failed" : status === "fetching" ? "Fetching repository" : "Indexing repository";
  return <div className={`index-summary ${complete ? "" : failed ? "index-failed" : "index-active"}`}>
    {complete ? <CircleCheck/> : failed ? <CircleAlert/> : <LoaderCircle className="index-spinner"/>}
    <span><strong>{title}</strong><small>{project?.files_indexed ?? 0} files · {project?.chunks_indexed ?? 0} chunks{typeof percent === "number" ? ` · ${percent}%` : ""}</small></span>
    {!complete && !failed && <div className={`index-progress ${typeof percent === "number" ? "determinate" : "indeterminate"}`} role="progressbar" aria-label={title} aria-valuemin={0} aria-valuemax={100} aria-valuenow={typeof percent === "number" ? percent : undefined}><i style={typeof percent === "number" ? { width: `${Math.max(0, Math.min(100, percent))}%` } : undefined}/></div>}
    <details><summary>View indexing details</summary><p>{project?.status_message || "Checking repository status…"}</p>{project?.current_file && <p className="current-index-file">{project.current_file}</p>}</details>
  </div>;
}
...
function UsageCostModal({ usage }: { usage:any }) {
  ...
  return <section className="usage-cost-modal"><nav className="usage-tabs">{["overview","models","operations","executions"].map(name=><button className={tab===name?"active":""} onClick={()=>setTab(name)} key={name}>{readable(name)}</button>)}</nav>
    {tab==="overview"&&<><div className="usage-overview">{[["Requests",o.requests,"Actual"],["Actual input tokens",o.actual_input_tokens,"Actual"],["Actual output tokens",o.actual_output_tokens,"Actual"],["Actual total tokens",o.actual_total_tokens,"Actual"],["Cached tokens",o.cached_tokens,"Actual"],["Actual provider API cost",o.actual_provider_api_cost,"Actual"],["Local generation time",o.local_model_generation_time_ms,"Actual"]].map(([label,v,kind])=><article key={String(label)}><small>{label}</small><strong>{String(label).includes("cost")?money(v):value(v,label==="Local generation time"&&v!=null?" ms":"")}</strong><span>{v==null?"Unavailable":kind}</span></article>)}</div><p className="pricing-note">Provider API cost uses the billed cost reported by OpenRouter when available. Older executions may show Unavailable because that value was not previously recorded. Local hardware and electricity cost are outside the current measurement scope.</p></>}
